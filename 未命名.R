...
apollo_fixed = c("asc_rs",
                 "b_rsa","b_rsa_shift","b_rsa_shift2","scale_best"
)
...
for (i in names(apollo_beta)){
  if (i %in% apollo_fixed|i == 'scale_worst') {}else{
    con = c(con,paste0(i," > -30"))
    con = c(con,paste0(i," < 30"))}
}
con = c(con,paste0("scale_worst > 0"))
#con = c(con,paste0("scale_worst < 1"))
...
predictions_base=p2p(predictions_base[["model"]][["at_estimates"]],TRUE)
...
v = c('mode_2023', 'qhousehold_3', 
      ...
      'age', 'edu', 'cs_rs_1_2'
      ,'att_pt_1', 'att_pt_2', 'att_pt_3', 'att_pt_4','time_living','qfamily_2', 'qfamily_3', 'qfamily_5'
      #, 'income_d'
) ###
