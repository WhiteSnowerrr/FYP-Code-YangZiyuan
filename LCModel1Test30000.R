...
p2p=function(pre, best=TRUE){
  output = pre[,c(1,2,9)]
  if (best){
    output$pt = pre$alt_bpt_wcs + pre$alt_bpt_wrs
    output$cs = pre$alt_bcs_wpt + pre$alt_bcs_wrs
    output$rs = pre$alt_brs_wpt + pre$alt_brs_wcs
  } else {
    output$pt = pre$alt_bcs_wpt + pre$alt_brs_wpt
    output$cs = pre$alt_bpt_wcs + pre$alt_brs_wcs
    output$rs = pre$alt_bpt_wrs + pre$alt_bcs_wrs
  }
  for (i in 1:nrow(pre)){
    if (best){
      if (pre[i,"chosen"] == pre[i,"alt_bpt_wcs"]|pre[i,"chosen"] == pre[i,"alt_bpt_wrs"]){
        output[i,"chosen"] = output[i,"pt"]
      } else if (pre[i,"chosen"] == pre[i,"alt_bcs_wpt"]|pre[i,"chosen"] == pre[i,"alt_bcs_wrs"]){
        output[i,"chosen"] = output[i,"cs"]
      } else if (pre[i,"chosen"] == pre[i,"alt_brs_wpt"]|pre[i,"chosen"] == pre[i,"alt_brs_wcs"]){
        output[i,"chosen"] = output[i,"rs"]
      }
    } else {
      if (pre[i,"chosen"] == pre[i,"alt_bcs_wpt"]|pre[i,"chosen"] == pre[i,"alt_brs_wpt"]){
        output[i,"chosen"] = output[i,"pt"]
      } else if (pre[i,"chosen"] == pre[i,"alt_bpt_wcs"]|pre[i,"chosen"] == pre[i,"alt_brs_wcs"]){
        output[i,"chosen"] = output[i,"cs"]
      } else if (pre[i,"chosen"] == pre[i,"alt_bpt_wrs"]|pre[i,"chosen"] == pre[i,"alt_bcs_wrs"]){
        output[i,"chosen"] = output[i,"rs"]
      }
    }
  }
  output = output[,c(1,2,4,5,6,3)]
  return(output)
}
...
apollo_fixed = c("asc_rs","b_rsa","b_rsa_shift","b_rsa_shift2","scale_best")
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  for(s in 1:3){
    ...
    mnl_settings$utilities     = list(alt_bpt_wcs=scale_best*V[["pt"]]-scale_worst*V[["cs"]],
...
estimate_settings = list(constraints = c("b_share_shift < 100"),
                         writeIter = FALSE)
...
predictions_base=p2p(predictions_base[["model"]][["at_estimates"]],be)
...
deltaMethod_settings=list(expression=c(VTT_Class_1="b_tt/b_cost",
                                       VTT_Class_2="(b_tt+b_tt_shift)/(b_cost+b_cost_shift)"))
...
for (i in y){
  ...
  for (s in 1:2){
    ...
    base=p2p(predictions_base[[paste0("Class_",s)]][["at_estimates"]],be)
    new=p2p(predictions_new[[paste0("Class_",s)]],be)
...
for (i in y){
  ...
  for (s in 1:2){
    #pi = (predictions_new[["model"]][,z]-predictions_new[[paste0("Class_",ifelse(s == 2, 1, 2))]][,z])/(predictions_new[[paste0("Class_",s)]][,z]-predictions_new[[paste0("Class_",ifelse(s == 2, 1, 2))]][,z])
    revenue = p2p(predictions_new[[paste0("Class_",s)]],be)[,z]*database[,x]*scale*i
    base=p2p(predictions_base[[paste0("Class_",s)]][["at_estimates"]],be)
    new=p2p(predictions_new[[paste0("Class_",s)]],be)
    change=(new-base)/base
    change=change[,-ncol(change)]
  ...
  revenue = p2p(predictions_new[["model"]],be)[,z]*database[,x]*scale*i
...
for (s in 1:2){
  base=p2p(predictions_base[[paste0("Class_",s)]][["at_estimates"]],be)
  new=p2p(predictions_new[[paste0("Class_",s)]][["at_estimates"]],be)
...
sharesTest_settings[["alternatives"]] = c(alt_bpt_wcs=12,
                                          alt_bpt_wrs=13, 
                                          alt_bcs_wpt=21, 
                                          alt_bcs_wrs=23,
                                          alt_brs_wpt=31,
